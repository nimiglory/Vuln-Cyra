...
def validate_url(url):
    """Validate and normalize URL without extra dependencies."""
    parsed = urlparse(url)
...
def scan_sql_injection(url):
    """
    Naive SQL Injection test without sqlmap.
    Appends common payloads to parameters and checks responses.
    """
...
def scan_xss(url):
    """Test query params for XSS vulnerabilities."""
...
def detect_safe_thread_count():
    """Detect CPU threads and set safe max workers."""
...
def scan_open_ports(host, ports=None):
    """
    Scan open ports using python-nmap (no binary path needed).
    ...
    try:
        ...
        if ports:
            ...
            nm.scan(host, arguments="-p-")  # full TCP scan
        ...
        if ports is None:
            ports = range(1, 1025)  # limit default scan to top 1024 for performance
